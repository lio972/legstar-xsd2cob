...
public class Java2CobModelTest extends AbstractTest {
    ...
    public void testSerialize() {
        ...
        assertTrue(props.getProperty(Java2CobModel.NEW_TARGET_NAMESPACE)
                .contains("uri:some/namespace"));

        List < String > classNames = Arrays.asList(new String[] {
                "org.FirstClass", "org.SecondClass" });
        ...
        List < String > pathElementLocations = Arrays.asList(new String[] {
                "bin", "target/classes" });
        model.setPathElementLocations(pathElementLocations);
        props = model.toProperties();
        assertEquals("bin", props.getProperty("pathElementLocations_0"));
        assertEquals("target/classes",
                props.getProperty("pathElementLocations_1"));
    }
    ...
    public void testDeserialize() {
        ...
        props.setProperty(Java2CobModel.NEW_TARGET_NAMESPACE,
                "uri:some/namespace");
    ...
    public void testAntScriptGeneration() throws Exception {
        ...
        runAnt(resultFile);
        check("jvmquery", "xsd", FileUtils.readFileToString(new File(
                GEN_XSD_DIR, "jvmquery.xsd"), "UTF-8"));
        check("jvmquery", "cpy", FileUtils.readFileToString(new File(
                GEN_COBOL_DIR, "jvmquery.cpy"), "UTF-8"));
    }
     ...
     */
    protected File genAntScriptAsFile(final Java2CobModel model)
            throws Exception {
