...
/**
 * Reads XML Schema from an input stream producing an Apache XML Schema
 * object;
 *
 */
public class XsdReader {
	 ...
	 * @throws InvalidDocumentException if there is something wrong in the document
	 */
	public static XmlSchema read(
			final Document doc) throws InvalidDocumentException {

        Element definitionsElement = null;
        Element schemaElement = null;

        NodeList nodes = doc.getElementsByTagNameNS(XsdConstants.WSDL_NS, "definitions");
        ...
        if (schemaElement == null) {
        	throw new InvalidDocumentException("Unable to locate an XML Schema in input document");
        }
	...
	public static void pullIncludes(final XmlSchema xmlSchema) {
		...
		for (Iterator includedItems = xmlSchema.getIncludes().getIterator(); includedItems
			...
			if (includeOrImport instanceof XmlSchemaInclude) {
				...
				XmlSchema incSchema = ((XmlSchemaInclude) includeOrImport).getSchema();
				XmlSchemaObjectCollection incItems = incSchema.getItems();
				for (Iterator incIt = incItems.getIterator(); incIt.hasNext();) {
					XmlSchemaObject incXsdObject = (XmlSchemaObject) incIt.next();
					if (incXsdObject instanceof XmlSchemaType) {
						QName name = getName(incSchema.getSchemaTypes(), incXsdObject);
						xmlSchema.getSchemaTypes().add(name, incXsdObject);
					} else if (incXsdObject instanceof XmlSchemaElement) {
						QName name = getName(incSchema.getElements(), incXsdObject);
     ...
     * @throws InvalidDocumentException if root elements cannot be added
	 */
    public static void addWsdlPartsAsRootElements(
    		final Document doc,
            final XmlSchema schema) throws InvalidDocumentException {

    	Map < QName, QName > rootElements = new HashMap < QName, QName >();

    	NodeList nodes = doc.getElementsByTagNameNS(XsdConstants.WSDL_NS, "part");
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (type != null && type.length() > 0) {
                ...
                rootElements.put(
                        new QName(schema.getTargetNamespace(), type),
     ...
     * existing elements.
     * <p/>
     * In case of name conflict we suffix the element name with
     * the type name in hope to disambiguate the name.
     * 
    ...
    public static void addRootElements(
    		final Map < QName, QName > rootElements,
    		final XmlSchema schema) throws InvalidDocumentException {
        ...
        for (QName typeQName : rootElements.keySet()) {
            ...
            if (schema.getTypeByName(typeQName) == null) {
                throw (new InvalidDocumentException("Root element " + elementQName
            ...
            if (el == null) {
            	...
            	QName newElementQName = new QName(elementQName.getNamespaceURI(),
     ...
     * @param schema the XML schema
     */
    public static void addXmlSchemaElement(
	 ...
	 * <p/>
	 * If the namespace is already there, returns the corresponding
	 * prefix.
	 * <p/>
